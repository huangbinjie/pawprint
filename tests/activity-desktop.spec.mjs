...
test("Codex lifecycle drives transient pet feedback without spending or replaying history", async ({}, info) => {
  ...
  try {
    ...
    await expect(floating.locator(".activity-bubble")).toHaveCount(0);
    await appendFile(file, row("task_started") + row("item_completed", { item: { type: "UserMessage", content: [{ type: "Text", text: "帮我修复 SECRET 的错误" }] } }));
    await expect(floating.locator(".activity-bubble")).toHaveText("陪你排查问题", { timeout: 10000 });
    ...
    await expect(floating.locator(".activity-bubble")).toHaveText("本轮回复结束啦", { timeout: 10000 });
    ...
    await expect(floating.locator(".activity-bubble")).toHaveCount(0, { timeout: 12000 });
